  ...
  return (
    <nav className="fixed top-0 left-0 right-0 flex items-center justify-between p-3 sm:p-4 md:p-5 bg-white shadow-md z-50">
      <div className="text-[#343434] flex items-center">
        <div className="text-4xl sm:text-5xl font-bold pl-2 sm:pl-4 md:pl-7">D</div>
      ...
      <div className="hidden md:flex space-x-6 lg:space-x-10 pr-4 lg:pl-20">
        <Link className={`text-lg ${getLinkClass("/")}`} to="/">Home</Link>
        <Link className={`text-lg ${getLinkClass("/events")}`} to="/events">Events</Link>
        <Link className={`text-lg ${getLinkClass("/about")}`} to="/about">About</Link>
      </div>
